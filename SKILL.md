...
Ask the owner for two things (or read them from your environment/config):

- `KEYHOLE_BASE_URL` — e.g. `http://localhost:8787` (local) or the deployed `https://<app>.vercel.app`. No trailing slash.
- `KEYHOLE_API_KEY` — starts with `kh_live_`.

```bash
export KEYHOLE_BASE_URL=http://localhost:8787
export KEYHOLE_API_KEY=kh_live_...
alias kh='curl -s -H "Authorization: Bearer $KEYHOLE_API_KEY" "$KEYHOLE_BASE_URL/api/v1"'
```
...
Machine-readable contract: `GET $KEYHOLE_BASE_URL/api/openapi.json` (OpenAPI 3.1).
